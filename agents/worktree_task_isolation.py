...
# -- Event Bus: append-only lifecycle events for observability --
class EventBus:
...
class TaskManager:
    ...
    def update(self, task_id: int, status: str = None, add_blocked_by: list = None, add_blocks: list = None, owner: str = None) -> str:
...
# -- WorkTree Manager: create/list/run/remove git worktrees + lifecycle index --
...
TOOL_HANDLERS = {
    ...
    "task_update": lambda **kw: TASKS.update(kw["task_id"], kw.get("status"), kw.get("addBlockedBy"), kw.get("addBlocks"), kw.get("owner")),
...
CHILD_TOOLS:list = [
    ...
    {"name": "task_update", "description": "Update a task's status or dependencies.",
     "input_schema": {"type": "object", "properties": {"task_id": {"type": "integer"}, "status": {"type": "string", "enum": ["pending", "in_progress", "completed"]}, "addBlockedBy": {"type": "array", "items": {"type": "integer"}}, "addBlocks": {"type": "array", "items": {"type": "integer"}}, "owner": {"type": "string"}}, "required": ["task_id"]}},
